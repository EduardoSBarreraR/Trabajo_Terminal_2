...
En donde $BH$[cm] es la distancia horizontal al punto más lejano, $BT$[cm] es la longitud del brazo totalmente estirado y $BV$[cm] es la distancia de la base del robot hasta el suelo.
...
En donde $V_{e}[m^3]$ es el volumen ocupado por el eslabón, $t_{e}[m]$ es el espesor del perfil del eslabón, $h_{e}$ es la altura del perfil del eslabón, $l_{e}[m]$ es la longitud del perfil del eslabón y $r_{e}[m]$ es el radio del chaflán del eslabón.
...
En donde $m_{e}[kg]$ es la masa del eslabón y $\rho_{e}[kg/m^3]$ es la densidad del eslabón.
...
En donde $F_{y}[N] =$ Fuerzas existentes en el eje Y.
...
En donde $M_{o}[Nm] =$ Momentos generados por las fuerzas normales al punto O.
...
Conociendo la corriente del motor bloqueado, y el par máximo se calcula la constante de par $K_{m}$[Nm/A] de acuerdo a la ecuación:

\begin{align*}
K_{m} = \frac{29.5[Nm]}{20[A]}=1.475\unitfrac{Nm}{A}
\end{align*}
...
Con estos pares calculados, y despejando para $i_{a}$[A] de la ecuación, se puede calcular la corriente esperada.

\begin{align}
i_{a} = \frac{\tau_{0}}{K_{m}}=\frac{11.145 Nm}{1.475\unitfrac{Nm}{A}} = 7.55[A]
\end{align}
...
En dónde $ Ic $[-] es el coeficiente de impacto, $L$[Kg] es la carga máxima a medir, $ L0 $[Kg] es la carga inicial (Peso de la superficie de medición), $ Lec $[-] es el coeficiente de excentricidad de carga, $ Lic $[-] coeficiente de desbalance de carga, $ Nc $[-] numero de celdas de carga a utilizar. 
